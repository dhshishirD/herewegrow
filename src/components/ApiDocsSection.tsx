...
export const ApiDocsSection: React.FC = () => {
  ...
  const codeSnippets = {
    curl: `curl -X POST https://api.herewegrow.pro/v2 \\
...
url = "https://api.herewegrow.pro/v2"
payload = {
    "key": "${apiKey}",
    "action": "add",
    "service": "fb-001",
...
async function createOrder() {
  const { data } = await axios.post('https://api.herewegrow.pro/v2', {
    key: '${apiKey}',
    action: 'add',
    service: 'fb-001',
    link: 'https://facebook.com/yourbrand',
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://api.herewegrow.pro/v2',
    ...
    <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-slate-400">Your Personal Reseller API Key</div>
            <div className="font-mono text-sm font-bold text-white mt-0.5">{apiKey}</div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold border border-white/10 flex items-center gap-1.5 transition-all"
          >
      ...
      {/* Code Snippet & Docs Box */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Left: Actions List */}
        <div className="glass-panel p-6 rounded-3xl border border-white/10 space-y-4">
          <h3 className="text-sm font-black text-white uppercase tracking-wider">Supported API Actions</h3>
          
          <div className="space-y-2.5 text-xs">
            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-1">
              <div className="font-mono font-bold text-emerald-400">action: "services"</div>
              <p className="text-slate-400 text-[11px]">List all active services, rates, min/max limits & categories.</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-1">
              <div className="font-mono font-bold text-indigo-400">action: "add"</div>
              <p className="text-slate-400 text-[11px]">Place a new automated order with service ID, link, and quantity.</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-1">
              <div className="font-mono font-bold text-pink-400">action: "status"</div>
              <p className="text-slate-400 text-[11px]">Check real-time delivery status, remains, and start count.</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-1">
              <div className="font-mono font-bold text-amber-400">action: "refill"</div>
              <p className="text-slate-400 text-[11px]">Trigger instant automated replacement refill for dropped counts.</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-1">
              <div className="font-mono font-bold text-cyan-400">action: "balance"</div>
              <p className="text-slate-400 text-[11px]">Retrieve current wallet balance in BDT & USD.</p>
            </div>
          </div>
        </div>

        {/* Right: Code Switcher & Response */}
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-slate-900 rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 bg-slate-950/80">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-300">Endpoint: https://api.herewegrow.pro/v2</span>
              </div>
            ...
            {/* Code Body */}
            <div className="p-5 relative bg-[#070B14]">
              <button
                onClick={() => handleCopyCode(codeSnippets[activeLang])}
                className="absolute top-4 right-4 p-2 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-all"
                title="Copy Snippet"
              >
          ...
          <div className="bg-slate-900 rounded-3xl border border-white/10 overflow-hidden">
            <div className="px-5 py-2.5 border-b border-white/10 bg-slate-950/80 flex items-center justify-between text-xs font-bold text-slate-400">
              <span>JSON Response (HTTP 200 OK)</span>
              <span className="text-emerald-400">Response time: ~42ms</span>
            </div>
            <div className="p-5 bg-[#070B14]">
