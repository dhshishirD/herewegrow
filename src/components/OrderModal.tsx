    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-start gap-3 mb-6 pr-8">
          <div className="w-11 h-11 rounded-2xl bg-indigo-50 text-indigo-700 flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">
        ...
        {/* Error Alert */}
        {errorMessage && (
          <div className="mb-4 p-3.5 rounded-xl bg-rose-50 border border-rose-200 flex items-center gap-2.5 text-xs text-rose-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-indigo-600"
            />
          ...
          {service && (
            <div>
              <div className="flex justify-between text-xs font-bold text-slate-700 mb-1">
                <span>Quantity</span>
                <span className="text-indigo-600 font-extrabold">{quantity.toLocaleString()} units</span>
              </div>
              <input
                type="number"
                min={service.minQty}
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:bg-white focus:border-indigo-600 mb-1.5"
              />
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
            ...
            <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200">
              ...
              <div className="flex items-center gap-2">
                <span className={`font-bold ${hasSufficientBalance ? 'text-slate-800' : 'text-rose-600'}`}>
                ...
                {!hasSufficientBalance && (
                  <button
                    ...
                    className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100"
                  >
                    + Top Up
                  </button>
                )}
          ...
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm shadow-xs flex items-center justify-center gap-2 transition-all disabled:opacity-50 hover:scale-[1.01]"
          >
            {isSubmitting ? (
